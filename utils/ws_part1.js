...
module.exports.process_msg = function(ws, data, io){
    ...
    if(data.type == 'newPro') {
        var value = data.pro_name + data.pro_num + data.pro_price + data.pro_desc + data.create_time;
        var sha = new jsSHA("SHA-256","TEXT");
        sha.update(value);
        var sha_value = sha.getHash("HEX");
        chaincode.invoke.createPro([data.pro_name, data.pro_num, data.pro_price, data.pro_desc,
            data.create_time], cb_invoked);
    }
